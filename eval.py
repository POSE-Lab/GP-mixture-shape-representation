...
class eval_args:
    ...
    # Metrics to compute
    metrics: list = dataclasses.field(
        default_factory=lambda: ["cd", "emd", "precision", "recall", "f1_score"]
    )

    # Points to evaluate each metric
    points_per_metric: list = dataclasses.field(
        default_factory=lambda: [30000, 500, -1, -1, -1]
    )
...
def run(args):
    ...
    for class_name in classes:
        make_dir(jn(common.MODELS_PATH, "est_models", class_name))
        model_dirs = [
            model
            for model in os.listdir(jn(common.RESULTS_PATH, class_name))
            if os.path.isdir(jn(common.RESULTS_PATH, class_name, model))
        ]
        for model in tqdm.tqdm(model_dirs, desc="Evaluating models"):
            try:

                    pcd_gt = load_point_cloud(
                        jn(common.MODELS_PATH, "test", class_name, model + ".ply")
                    )
                    # Points are already sampled from normalized meshes (see preprocess),
                    # so evaluate directly in that normalized space.

                    min_cd_error = float("inf")
                    best_rfp_metrics = None
                    best_rfp = None
                    for rfp in os.listdir(
                        jn(common.RESULTS_PATH, class_name, model.split(".")[0])
                    ):
                        if os.path.isdir(
                            jn(common.RESULTS_PATH, class_name, model.split(".")[0], rfp)
                        ): 
                            ...
                            rfp_metrics = metrics.compute(
                                pcd_est, pcd_gt, points_per_metric=args.points_per_metric
                            )
                    ...
                    if best_rfp_metrics is not None:
                        ...
                        output_csv_path = jn(
                            common.RESULTS_PATH, class_name, model, "best_metrics.csv"
                        )
                ...
                print(f"Skipping model {model}: {err}")
                continue
