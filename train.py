...
from utils.io import make_dir, log_event, log_debug
import gpytorch
...
def train_GPs_per_ref_point(
    ...
    def _extract_lengthscale(kernel):
        # Walk nested kernels (e.g., ScaleKernel -> ScaleKernel -> RQKernel)
        # and return the first available lengthscale.
        current = kernel
        for _ in range(6):
            if current is None:
                return float("nan")
            lengthscale = getattr(current, "lengthscale", None)
            if lengthscale is not None:
                return _safe_scalar(lengthscale)
            current = getattr(current, "base_kernel", None)
        return float("nan")
    ...
    with gpytorch.settings.max_cg_iterations(cg_iters):
        for cls in range(0, len(centers)):
            ...
            phis_thets_cls_train_t = torch.tensor(
                phis_thetas_train[cls], dtype=torch.float32, device="cuda"
            )
            ...
            optimizer = torch.optim.Adam(
                [
                    {"params": model.covar_module.parameters(), "lr": init_lr},
                    {"params": model.likelihood.parameters(), "lr": init_lr},
                ]
            )
            ...
            def closure():
                ...
                if log_loss:
                    lengthscale_value = _extract_lengthscale(model.covar_module)
                    noise_value = _safe_scalar(model.likelihood.noise)
                    log_debug(
